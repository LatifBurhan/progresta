...
export function ReportForm({ mode, reportId, initialData, onSuccess, onCancel }: ReportFormProps) {
  ...
  const loadProjects = async () => {
    ...
    } catch (error) { console.error('Load projects error:', error) }
    ...
    <form onSubmit={handleSubmit} className="space-y-10 max-w-4xl mx-auto pb-10">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Label className="text-base font-bold text-slate-800">Pilih Project <span className="text-rose-500">*</span></Label>
        </div>
        ...
        {errors.project_id && <p className="text-xs font-bold text-rose-500 flex items-center gap-1 mt-2"><AlertCircle className="w-3 h-3" /> {errors.project_id}</p>}
      </section>
      ...
      <section className="grid grid-cols-1 gap-8 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
        <div className="space-y-3">
          ...
          <Textarea
            ...
            disabled={loading}
          />
          {errors.pekerjaan_dikerjakan && <p className="text-xs font-bold text-rose-500 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {errors.pekerjaan_dikerjakan}</p>}
        </div>
      ...
      <footer className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-100">
        <Button
          type="submit"
          disabled={loading}
          className="flex-[2] h-12 rounded-xl bg-slate-900 hover:bg-black text-white font-bold shadow-xl shadow-slate-200 transition-all hover:-translate-y-0.5"
        >
          {loading ? (
            <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Menyimpan...</>
          ) : (
            mode === 'create' ? 'Kirim Laporan Sekarang' : 'Simpan Perubahan'
          )}
        </Button>
        <Button
          type="button"
          variant="outline"
          onClick={onCancel || (() => router.push('/reports'))}
          disabled={loading}
          className="flex-1 h-12 rounded-xl border-2 border-slate-100 font-bold text-slate-500 hover:bg-slate-50"
        >
          Batalkan
        </Button>
      </footer>

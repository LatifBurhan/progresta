    ...
    <div className="space-y-8 animate-in fade-in duration-700 text-slate-900">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-slate-500 max-w-md">Konfigurasi, monitoring, dan delegasi project tim Al-Wustho secara terpusat.</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-1">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button onClick={() => setCreateModalOpen(true)} className="h-13 px-8 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-xl shadow-blue-100 gap-2 active:scale-95 transition-all text-sm uppercase tracking-wider">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="ml-auto hidden md:block px-4 py-2 bg-blue-50 rounded-xl">
          <p className="text-xs font-bold text-blue-600 uppercase tracking-tight">Ditemukan: {filteredProjects.length} Project</p>
        </div>
      </div>
            ...
            >
              <div className="flex-1 space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg md:text-xl font-extrabold text-slate-900 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2">{project.name}</h3>
                </div>
                ...
                <div className="bg-white/60 p-4 rounded-[1.5rem] border border-slate-100">
                  ...
                  <p className="text-sm md:text-base text-slate-600 line-clamp-2 leading-relaxed font-medium italic">"{project.tujuan || "Fokus pada penyelesaian target operasional tim."}"</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 py-1">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm md:text-base font-semibold text-slate-700">{project.assignments && project.assignments.length > 0 ? `${project.assignments.length} Orang` : "Semua Anggota"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 pt-5 border-t border-slate-200/50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border ${getPriorityStyle(project.prioritas)}`}>{project.prioritas || "Normal"}</span>
                  {project.lampiran_files && project.lampiran_files.length > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        openFilePreview(project);
                      }}
      ...
      {filteredProjects.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 px-6 bg-white border-2 border-dashed border-slate-100 rounded-[3rem] text-center space-y-4 shadow-inner">
          ...
          <div className="space-y-2">
            ...
            <p className="text-base text-slate-400 font-medium max-w-sm mx-auto leading-relaxed">Kami tidak dapat menemukan data yang sesuai dengan filter atau kata kunci yang Anda masukkan.</p>
          </div>
          <Button
            variant="outline"
            onClick={() => {

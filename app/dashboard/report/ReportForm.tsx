...
export default function ReportForm({ userId, lastReport, availableProjects, initialHistory }: ReportFormProps) {
  ...
  const updateProjectDetail = (id: string, field: keyof ProjectDetail, value: any) => {
    setProjectDetails(projectDetails.map((p) => (p.id === id ? { ...p, [field]: value } : p)));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitReport({
        ...
        location,
        futurePlan,
        projectDetails: projectDetails.filter((p) => p.projectId && p.task.trim() && p.progress.trim()),
      });
      ...
      <div className="space-y-6">
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-green-800 mb-2">Laporan Berhasil Dikirim!</h3>
            <p className="text-green-700 text-sm">Progres kerja Anda telah tersimpan</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="bg-gray-50 p-3 rounded-lg text-sm font-mono mb-4 max-h-40 overflow-y-auto">{whatsappText}</div>
            <Button onClick={copyToWhatsApp} className="w-full bg-green-600 hover:bg-green-700">
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <h3 className="font-medium mb-3 flex items-center gap-2">🕒 Periode Waktu</h3>
          <div className="grid grid-cols-2 gap-2">
            {PERIODS.map((p) => (
              <button key={p.value} onClick={() => setPeriod(p.value)} className={`p-3 rounded-lg border-2 transition-all ${period === p.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 hover:border-gray-300"}`}>
      ...
      <Card>
        <CardContent className="p-4">
          <h3 className="font-medium mb-3 flex items-center gap-2">📍 Lokasi Kerja</h3>
          <div className="flex gap-2 flex-wrap">
            {LOCATIONS.map((loc) => (
              <Badge
                key={loc.value}
                variant={location === loc.value ? "default" : "outline"}
                className={`cursor-pointer px-4 py-2 text-sm ${location === loc.value ? "bg-blue-500 hover:bg-blue-600" : "hover:bg-gray-100"}`}
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-medium flex items-center gap-2">👨‍💻 Detail Project</h3>
            <Button onClick={addProjectDetail} size="sm" variant="outline" className="text-xs">
              ...
              <div key={detail.id} className="border rounded-lg p-4 bg-gray-50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {projectDetails.length > 1 && (
                    <button onClick={() => removeProjectDetail(detail.id)} className="text-red-500 text-xs hover:text-red-700">
                      Hapus
                    </button>
                  )}
                </div>
                ...
                {/* Project Selection */}
                <div className="mb-3">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Pilih Project Aktif</label>
                  <select
                ...
                {/* Task Description */}
                <div className="mb-3">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Yang Sudah Dikerjakan</label>
                  <textarea
                    value={detail.task}
                ...
                <div className="mb-3">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Progress/Hasil</label>
                  <textarea
                ...
                {/* Hours Spent */}
                <div className="mb-3">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Jam Kerja (dalam jam)</label>
                  <Input type="number" min="0.5" max="8" step="0.5" value={detail.hoursSpent} onChange={(e) => updateProjectDetail(detail.id, "hoursSpent", parseFloat(e.target.value) || 0)} className="w-full" />
                </div>
                {/* Evidence Upload */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Bukti Kerja (Foto)</label>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor={`evidence-camera-${detail.id}`} className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 text-sm">
                    ...
                    />
                    <label htmlFor={`evidence-gallery-${detail.id}`} className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 text-sm">
                      Pilih Galeri
                    </label>
                    {detail.evidence && <span className="text-xs text-green-600">✓ {detail.evidence.name}</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="mb-3">
            <label className="block text-sm font-medium text-gray-700 mb-1">Filter Project</label>
            <select value={historyProjectFilter} onChange={(e) => setHistoryProjectFilter(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {filteredHistory.length === 0 && <p className="text-sm text-gray-500">Belum ada riwayat laporan.</p>}
            ...
            {filteredHistory.map((report: any) => (
              <div key={report.id} className="border rounded-lg p-3 bg-gray-50">
                ...
                <div className="text-sm font-medium text-gray-800 mb-1">Total Jam: {report.totalHours} jam</div>
      ...
      <div className="sticky bottom-4 bg-white p-4 rounded-lg shadow-lg border">
        <Button onClick={handleSubmit} disabled={isSubmitting || !period} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 py-3 text-base font-medium">
          {isSubmitting ? (
            <div className="flex items-center gap-2">
        ...
        {!period && <p className="text-xs text-red-500 text-center mt-2">Pilih periode waktu terlebih dahulu</p>}
      </div>
    </div>
  );
}
